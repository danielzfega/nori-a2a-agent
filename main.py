...
app = FastAPI(title="Nori News Agent", version="1.2.0")


def clean_prompt(text: str) -> str:
    """Remove HTML tags & normalize input"""
            ...
            if p.get("kind") == "data" and isinstance(p.get("data"), list):
                for inner in p["data"]:
                    if inner.get("kind") == "text" and inner.get("text"):
                        return clean_prompt(inner["text"])
    except:
        pass
    return None
    ...
    try:
        ...
        if not articles:
            response_text = (
                ...
                "Try topics like: `tech`, `china tech`, `AI`, `startup news`, `crypto`"
            )
            raw_news = ""
            state = "completed"
        else:
...
@app.get("/health")
async def health():
    return {"status": "ok", "agent": "Nori", "version": "1.2.0"}
